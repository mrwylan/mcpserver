...
public class ColorTranslationService {
	...
	@McpTool(name = "my_mcp_colors", description = "Get a list of well known colors.")
	public List<HexColor> getWellknownColors() {
		return wellknownColors;
	}

	@McpTool(name = "my_mcp_color", description = "Get the single color name and hex value representation.")
	public HexColor getColorHex(
			@McpToolParam(description = "The name of the color to retrieve", required = true) String colorName) {
		if (colorName == null) {
			return DEFAULT_COLOR;
		}
		return wellknownColors.stream()
			.filter(hc -> hc.colorName().equalsIgnoreCase(colorName))
			.findFirst()
	 ...
	 */
	@McpTool(name = "my_mcp_add_color",
			description = "Add a new color with according hex representation to the service.")
	public boolean addColor(
			@McpToolParam(description = "The name of the color to add", required = true) String colorName,
			@McpToolParam(description = "The hex value of the color (format: #RRGGBB)",
					required = true) String hexValue) {
