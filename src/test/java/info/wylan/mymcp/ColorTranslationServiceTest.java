...
class ColorTranslationServiceTest {
	...
	void getWellknownColors() {
		...
		assertEquals(initialSize, colors.stream().distinct().count(), "Liste sollte keine Duplikate enthalten");

		// Überprüfe das Format aller Hex-Werte
		assertTrue(colors.stream().allMatch(color -> color.hexValue().matches("^#[0-9A-Fa-f]{6}$")),
	...
	void getColorHex() {
		...
		assertEquals("schwarz", nullFarbe.colorName(), "Null-Eingabe sollte DEFAULT_COLOR (schwarz) zurückgeben");
		assertEquals("#000000", nullFarbe.hexValue(), "Null-Eingabe sollte DEFAULT_COLOR (#000000) zurückgeben");
	...
	void addColor() {
		...
		assertEquals(initialSize + 1, service.getWellknownColors().size(), "Liste sollte um eine Farbe gewachsen sein");
		...
		assertAll(() -> assertFalse(service.addColor("ungültig1", "000000"), "Hex-Wert ohne # sollte abgelehnt werden"),
				...
				() -> assertFalse(service.addColor("ungültig5", null), "Null Hex-Wert sollte abgelehnt werden"));
		...
		assertTrue(service.addColor("hellblau", "#00ffFF"), "Hex-Wert mit Kleinbuchstaben sollte akzeptiert werden");
		HexColor hellblau = service.getColorHex("hellblau");
		assertEquals("#00ffFF", hellblau.hexValue(), "Hex-Wert sollte in Originalschreibweise gespeichert werden");

		// 4. Test: Leerer Farbname
		assertTrue(service.addColor("", "#CCCCCC"), "Leerer Farbname sollte akzeptiert werden");
		HexColor emptyName = service.getColorHex("");
		assertEquals("#CCCCCC", emptyName.hexValue(), "Farbe mit leerem Namen sollte auffindbar sein");
	}
